...
Map the first non-success return, then find the first related plog `ERROR` in the same time, process, and device window. A later synchronization or cleanup error must not replace that first failure.
