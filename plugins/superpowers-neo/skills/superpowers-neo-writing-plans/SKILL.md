...
A generated plan does not expand Git delivery authority. Automatic delivery authorizes a scoped commit by default and a normal push from an established, task-owned non-default branch; outside automatic worktree isolation, branch choice on the default branch and pull-request creation still require user direction. Explicitly invoking `superpowers-neo-git-delivery` is a separate bundled authorization for task-branch creation, commit, normal push, and PR creation. Merge, history rewrite, hook bypass, and cleanup always retain their own explicit authorization boundaries. Treat adding any protected action as a material approval decision, not ordinary plan detail.
